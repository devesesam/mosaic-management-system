/*
  # Fix database security and ensure worker profiles
  ...
  2. Functions
     - Recreate worker deletion function with proper parameter handling
     - Create function to repair worker profiles for all auth users
*/
...
RETURNS TABLE (email_out text, status text) AS $$
DECLARE
  auth_user RECORD;
BEGIN
  FOR auth_user IN (SELECT * FROM auth.users) LOOP
    -- Check if worker exists
    IF NOT EXISTS (SELECT 1 FROM public.workers WHERE workers.email = auth_user.email) THEN
      -- Create worker
