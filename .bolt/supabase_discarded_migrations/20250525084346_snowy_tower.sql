-- EMERGENCY FIX: Add direct sample data that will show in the UI
...
-- Get the first worker ID to use for job assignments
DO $$
DECLARE
  worker1_id UUID;
  worker2_id UUID;
...
-- Set all database constraints to deferred to avoid any conflicts
ALTER TABLE jobs ALTER CONSTRAINT jobs_pkey DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE workers ALTER CONSTRAINT workers_pkey DEFERRABLE INITIALLY IMMEDIATE;
ALTER TABLE job_secondary_workers ALTER CONSTRAINT job_secondary_workers_pkey DEFERRABLE INITIALLY IMMEDIATE;