/*
  ...
  1. Changes
    - Permanently disable RLS (Row Level Security) on all tables
    - Drop all security policies
    - Remove role-based access controls
    - Make all data visible to everyone
  2. Security
    - WARNING: This disables all security restrictions
    - Anyone who logs in will have full access to all data
*/
...
-- Remove role check constraint from users table
...
-- Remove role check constraint from workers table  
...
-- Update all existing users and workers to be admins
