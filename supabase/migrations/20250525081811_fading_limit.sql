/*
  # Fix data access and add diagnostics

  1. Changes:
    - Create worker profile for admin user
    - Update RLS policies for all tables to allow full access
    - Fix diagnostic functions to handle bigint/integer type mismatch
    - Add sample data for testing
    - Create user-worker relationship functions
*/
...
-- Check if jobs are visible - FIX: explicitly cast to integer to avoid type mismatch
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*)::integer FROM job_secondary_workers);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
